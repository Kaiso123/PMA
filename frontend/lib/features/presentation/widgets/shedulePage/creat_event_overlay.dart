...
OverlayEntry createEventOverlayEntry(
  ...
  final ValueNotifier<String> currentLocation = ValueNotifier('');
  final ValueNotifier<bool> currentIsAllDay = ValueNotifier(false);
  ...
  final ValueNotifier<String> currentEndTime = ValueNotifier(DateTime.now().add(const Duration(hours: 1)).toIso8601String());
  final ValueNotifier<DateTime> selectedCreatedDate = ValueNotifier(DateTime.now());
  final ValueNotifier<DateTime> selectedEndDate = ValueNotifier(DateTime.now().add(const Duration(hours: 1)));
  final ValueNotifier<List<int>> currentUserIds = ValueNotifier([]);
                      ...
                      child: GestureDetector(
                        ...
                        child: ValueListenableBuilder<String>(
                          ...
                          builder: (context, value, child) {
                            return buildDetailRow(
                              'End Time',
                              value.isEmpty ? 'Select Date' : DateFormat('yyyy-MM-dd HH:mm').format(DateTime.parse(value)),
                            );
                          },
                        ),
                      ),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            final newEvent = Event(
                              ...
                              createdBy: 1, // Giả sử userId hiện tại
